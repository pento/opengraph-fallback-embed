...
function Edit( { attributes, setAttributes } ) {
	...
	const handleSubmit = useCallback( () => {
		...
		try {
			...
			setError(
				__( 'Please enter a valid URL.', 'opengraph-fallback-embed' )
			);
			return;
		}
	...
	const sidebar = (
		<InspectorControls>
			<PanelBody
				title={ __( 'Embed Settings', 'opengraph-fallback-embed' ) }
			>
				<TextControl
	...
	return (
		<div { ...blockProps }>
			...
			<Placeholder
				icon="share-alt"
				label={ __( 'OpenGraph Embed', 'opengraph-fallback-embed' ) }
				instructions={ __(
			...
			>
				...
				{ error ? (
					<p className="og-fallback-embed-input-error">{ error }</p>
				) : null }
			</Placeholder>
		</div>
	);
